...
MAKE_PRINT_X(U4, printf, "%u")
MAKE_PRINT_X(U8, printf, "%u")
...
for (i=1; i<argc; ++i) {
	...
	while ((rec=stdf_read_record(f)) != NULL) {
		...
		switch (HEAD_TO_REC(rec->header)) {
			...
			case REC_PSR: {
                              rec_psr *psr = (rec_psr*) rec;
                              print_hex("COUNT_FLAG", psr->COUNT_FLAG);
                              print_int("PSR_INDX", psr->PSR_INDX);
                              print_str("PSR_NAM", psr->PSR_NAM);
                              print_hex("OPT_FLG", psr->OPT_FLG);
			...
			case REC_FTR: {
				...
				break;
                        ...
                        case REC_STR: {
                                rec_str *record = (rec_str*)rec;
                                print_int("CONT_FLG", record->CONT_FLG);
                                print_int("TEST_NUM", record->TEST_NUM);
                                print_int("HEAD_NUM", record->HEAD_NUM);
                                print_int("SITE_NUM", record->SITE_NUM);
                                print_int("PSR_REF", record->PSR_REF);
                                print_int("TEST_FLG", record->TEST_FLG);
                                print_str("LOG_TYP", record->LOG_TYP);
                                print_str("TEST_TXT", record->TEST_TXT);
                                print_str("ALARM_ID", record->ALARM_ID);
                                print_str("PROG_TXT", record->PROG_TXT);
                                print_str("RSLT_TXT", record->RSLT_TXT);
                                print_int("Z_VAL", record->Z_VAL);
                                print_hex("FMU_FLG", record->FMU_FLG);
                                if(record->hasMaskMap) {
                                  print_Dn("MASK_MAP", record->MASK_MAP);
                                }
                                if(record->hasFalMap) {
                                  print_Dn("FAL_MAP", record->FAL_MAP);
                                }

                                print_int("CYC_CNT", record->CYC_CNT);
                                print_int("TOTF_CNT", record->TOTF_CNT);
                                print_int("TOTL_CNT", record->TOTL_CNT);
                                print_int("CYC_BASE", record->CYC_BASE);
                                print_int("BIT_BASE", record->BIT_BASE);
                                print_int("COND_CNT", record->COND_CNT);
                                print_int("LIM_CNT", record->LIM_CNT);
                                print_int("CYC_SIZE", record->CYC_SIZE);
                                print_int("PMR_SIZE", record->PMR_SIZE);
                                print_int("CHN_SIZE", record->CHN_SIZE);
                                print_int("PAT_SIZE", record->PAT_SIZE);
                                print_int("BIT_SIZE", record->BIT_SIZE);
                                print_int("U1_SIZE", record->U1_SIZE);
                                print_int("U2_SIZE", record->U2_SIZE);
                                print_int("U3_SIZE", record->U3_SIZE);
                                print_int("UTX_SIZE", record->UTX_SIZE);
                                print_int("CAP_BGN", record->CAP_BGN);
                                print_xU2("LIM_INDX", record->LIM_INDX, record->LIM_CNT);
                                print_xU4("LIM_SPEC", record->LIM_SPEC, record->LIM_CNT);
                                print_xCn("COND_LST", record->COND_LST, record->COND_CNT);

                                print_int("CYCO_CNT", record->CYCO_CNT);
                                print_Vn("CYC_OFST", record->CYC_OFST, record->CYCO_CNT);

                                print_int("PMR_CNT", record->PMR_CNT);
                                print_Vn("PMR_INDX", record->PMR_INDX, record->PMR_CNT);

                                print_int("CHN_CNT", record->CHN_CNT);
                                print_Vn("CHN_NUM", record->CHN_NUM, record->CHN_CNT);

                                print_int("EXP_CNT", record->EXP_CNT);
                                print_Vn("EXP_DATA", record->EXP_DATA, record->EXP_CNT);

                                print_int("CAP_CNT", record->CAP_CNT);
                                print_Vn("CAP_DATA", record->CAP_DATA, record->CAP_CNT);

                                print_int("NEW_CNT", record->NEW_CNT);
                                print_Vn("NEW_DATA", record->NEW_DATA, record->NEW_CNT);

                                print_int("PAT_CNT", record->PAT_CNT);
                                print_Vn("PAT_NUM", record->PAT_NUM, record->PAT_CNT);

                                print_int("BPOS_CNT", record->BPOS_CNT);
                                print_Vn("BIT_POS", record->BIT_POS, record->BPOS_CNT);

                                print_int("USR1_CNT", record->USR1_CNT);
                                print_Vn("USR1", record->USR1, record->USR1_CNT);

                                print_int("USR2_CNT", record->USR2_CNT);
                                print_Vn("USR2", record->USR2, record->USR2_CNT);

                                print_int("USR3_CNT", record->USR3_CNT);
                                print_Vn("USR3", record->USR3, record->USR3_CNT);

                                print_int("TXT_CNT", record->TXT_CNT);
                                print_Vn("USER_TXT", record->USER_TXT, record->TXT_CNT);
                                break;
                        }
			case REC_BPS: {
