...
static int _stdf_header_to_idx(rec_typ type, rec_sub sub)
{
	switch (MAKE_REC(type, sub)) {
		...
		case REC_PSR:   return 32;
		case REC_STR:   return 33;
		case REC_UNKNOWN:
		default:        return 0;
	}
}
		...
		case REC_FTR: {
			...
			break;
                ...
                case REC_STR: {
                        ...
                        free_xU2(str->LIM_SPEC);
                        free(str->COND_LST);
                        free_Vn(str->CYC_OFST, str->CYCO_CNT);
                        free_Vn(str->PMR_INDX, str->PMR_CNT);
                        free_Vn(str->CHN_NUM, str->CHN_CNT);
                        free_Vn(str->EXP_DATA, str->EXP_CNT);
                        free_Vn(str->CAP_DATA, str->CAP_CNT);
                        free_Vn(str->NEW_DATA, str->NEW_CNT);
                        free_Vn(str->PAT_NUM, str->PAT_CNT);
                        free_Vn(str->BIT_POS, str->BPOS_CNT);
                        free_Vn(str->USR1, str->U1_SIZE);
                        free_Vn(str->USR2, str->U2_SIZE);
                        free_Vn(str->USR3, str->U3_SIZE);
                        free_Vn(str->USER_TXT, str->TXT_CNT);
                        free(rec);
                        break;
                }
		case REC_MPR: {
...
rec_psr* stdf_read_rec_psr(stdf_file *file)
{
        rec_psr *psr = __malloc_rec(rec_psr);
        _stdf_read_dtc_B1(file, &(psr->COUNT_FLAG));
...
rec_str* stdf_read_rec_str(stdf_file *file)
{
        rec_str *str = __malloc_rec(rec_str);
        memset(str, 0, sizeof(rec_str));
        ...
        int bit0 = 1 & str->FMU_FLG;
        int bit1 = (1 < 1) & str->FMU_FLG;
        int bit2 = (1 < 2) & str->FMU_FLG;
        int bit3 = (1 < 3) & str->FMU_FLG;
        if(bit0 == 1 && bit1 == 0) {
          str->hasMaskMap = 1;
        ...
        _stdf_read_dtc_U8(file, &(str->CYC_CNT));
        ...
        _stdf_read_dtc_U1(file, &(str->CYC_SIZE));
        ...
        _stdf_read_dtc_U2(file, &(str->CYCO_CNT));
        _stdf_read_dtc_Vn2(file, &(str->CYC_OFST), str->CYCO_CNT, str->CYC_SIZE);

        _stdf_read_dtc_U2(file, &(str->PMR_CNT));
        _stdf_read_dtc_Vn2(file, &(str->PMR_INDX), str->PMR_CNT, str->PMR_SIZE);

        _stdf_read_dtc_U2(file, &(str->CHN_CNT));
        _stdf_read_dtc_Vn2(file, &(str->CHN_NUM), str->CHN_CNT, str->CHN_SIZE);

        _stdf_read_dtc_U2(file, &(str->EXP_CNT));
        _stdf_read_dtc_Vn2(file, &(str->EXP_DATA), str->EXP_CNT, 1);

        _stdf_read_dtc_U2(file, &(str->CAP_CNT));
        _stdf_read_dtc_Vn2(file, &(str->CAP_DATA), str->CAP_CNT, 1);

        _stdf_read_dtc_U2(file, &(str->NEW_CNT));
        _stdf_read_dtc_Vn2(file, &(str->NEW_DATA), str->NEW_CNT, str->PAT_SIZE);

        _stdf_read_dtc_U2(file, &(str->PAT_CNT));
        _stdf_read_dtc_Vn2(file, &(str->PAT_NUM), str->PAT_CNT, str->PAT_SIZE);

        _stdf_read_dtc_U2(file, &(str->BPOS_CNT));
        _stdf_read_dtc_Vn2(file, &(str->BIT_POS), str->BPOS_CNT, str->BIT_SIZE);

        _stdf_read_dtc_U2(file, &(str->USR1_CNT));
        _stdf_read_dtc_Vn2(file, &(str->USR1), str->USR1_CNT, str->U1_SIZE);

        _stdf_read_dtc_U2(file, &(str->USR2_CNT));
        _stdf_read_dtc_Vn2(file, &(str->USR2), str->USR2_CNT, str->U2_SIZE);

        _stdf_read_dtc_U2(file, &(str->USR2_CNT));
        _stdf_read_dtc_Vn2(file, &(str->USR2), str->USR3_CNT, str->U3_SIZE);

        _stdf_read_dtc_U2(file, &(str->TXT_CNT));
        _stdf_read_dtc_Vn2(file, &(str->USER_TXT), str->TXT_CNT, str->UTX_SIZE);
        return str;
}
...
static inline size_t _calc_rec_len_psr(attribute_unused stdf_file *f, rec_psr *r)
{
        return
                sizeof(r->COUNT_FLAG) + sizeof(r->PSR_INDX) +
                _lenCn(r->PSR_NAM) + sizeof(r->OPT_FLG) + sizeof(r->TOTP_CNT) + sizeof(r->LOCP_CNT) +
...
static inline size_t _calc_rec_len_str(stdf_file *f, rec_str *r)
{
  //TODO
  return 0;
//  return
...
//  sizeof(r->UTX_SIZE)+
//  sizeof(r->CAP_BGN)+
//  _len_dtcX(r->LIM_INDX, r->LIM_CNT)+
//  _len_dtcX(r->LIM_SPEC, r->LIM_CNT)+
//  _len_dtcXCN(r->COND_LST, r->COND_CNT)+
//  sizeof(r->CYCO_CNT)+
//  _lenVn(r->CYC_OFST)+
//  sizeof(r->PMR_CNT)+
//  _lenVn(r->PMR_INDX)+
...
ssize_t stdf_write_rec_psr(stdf_file *file, rec_psr *psr)
{
        if (!psr->header.REC_LEN)
                SET_HEADER(psr->header, REC_SDR, _calc_rec_len_psr(file, psr));
        //TODO
//        _stdf_check_write_buffer(file, sdr->header.REC_LEN);
//        _stdf_write_dtc_header(file, &(sdr->header));
//        _stdf_write_dtc_U1(file, sdr->HEAD_NUM);
...
ssize_t stdf_write_rec_str(stdf_file *file, rec_str *str)
{
  //TODO
  return 0;
//        if (!str->header.REC_LEN)
//                SET_HEADER(str->header, REC_FTR, _calc_rec_len_str(file, str));
