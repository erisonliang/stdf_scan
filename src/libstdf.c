...
rec_unknown* stdf_parse_raw_record(rec_unknown *raw_rec)
{
	...
	switch (HEAD_TO_REC(file->header)) {
		...
		case REC_STR: rec = (rec_unknown*)stdf_read_rec_str(file); break;
		...
		case REC_PSR: rec = (rec_unknown*)stdf_read_rec_psr(file); break;
...
ssize_t stdf_write_record(stdf_file *file, void *rec_void)
{
	...
	switch (HEAD_TO_REC(rec->header)) {
		...
		case REC_PSR: return stdf_write_rec_psr(file, (rec_psr*)rec);
